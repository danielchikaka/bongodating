...
if($_REQUEST['btn_delete']=="Delete"){ 
  if(is_array($_REQUEST['chk_id'])){ 
    foreach($_REQUEST['chk_id'] as $id){
	  $sql="delete from classified_cat  where id='$id'"; 
	  $rs=mysql_query($sql); 
...
if($_REQUEST['btn_delete']=="Activate"){ 
  if(is_array($_REQUEST['chk_id'])){ 
    foreach($_REQUEST['chk_id'] as $id){ 
	  $sql="update classified_cat  set status='Y' where id='$id'"; 
	  $rs=mysql_query($sql); 
...
if($_REQUEST['btn_delete']=="Deactivate"){ 
  if(is_array($_REQUEST['chk_id'])){
    foreach($_REQUEST['chk_id'] as $id){ 
	  $sql="update classified_cat  set status='N' where id='$id'"; 
	  $rs=mysql_query($sql); 
...
<table width="100%" border="0" align="center" cellspacing="0" cellpadding="0">
  <form name="frmvincent" method="post" action="user.php?frm=classified_cat_edit" enctype="multipart/form-data"> 
  <tr valign="top">
    <td>
	  <table width="100%" align="center" border="0" cellspacing="1" cellpadding="1">
		...
		<?
		$t_rec=15;
		$sql="select * from classified_cat  order by id desc";
		...
		while($a_row=mysql_fetch_object($rs)){
		  ...
		  <img src="../images/edit.gif" onclick="javascript:location.href='user.php?frm=classified_cat_modify&id=<?=$id?>';" border="0" style="cursor:pointer;" />
		   <? 
		  print "</td>";
		...
		for($pgno=1;$pgno<$tot_pgs;$pgno++){ 
		  ...
		  print "<a href='user.php?frm=catogries_edit&strt=".$strt."&pgn=".$pgno."' class='$currpg'>".$pgno."</a> ";
			$chk_pg=$chk_pg+1;
		}
