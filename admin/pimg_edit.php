...
if($_REQUEST['btn_delete']=="Delete"){ 
  if(is_array($_REQUEST['chk_id'])){ 
	...
	{
	//print_r($id); exit;
	  $sql="select * from tbl_pimg where id ='$id'"; 
	  $rs=mysql_query($sql);
	  $row=mysql_fetch_object($rs);
	  @unlink("slide/thumb/".$row->pname); 
	  @unlink("slide/".$row->pname); 
	  $sqld = "delete from tbl_pimg where id='$id'";
	  mysql_query($sqld);
	  $err="Image(s) Deleted Successfully !!"; 
	} 
  } ?><script>javascript:location.href="user.php?frm=pimg_edit&err=<?=$err?>";</script><? 
}
if($_REQUEST['btn_update']=="Update")
{
  if(is_array($_REQUEST['chk_id']))
  { 
   $i=0;

    foreach($_REQUEST['hidden_id'] as $id)
	{
	
	//print_r($id); exit;
 $sql="update  tbl_pimg set title='".trim(addslashes($_POST['title2'][$i]))."' where id ='$id'"; 
	  mysql_query($sql);
	    $i++;
...
<tr valign="top"><td>
  ...
  <form name="frmvincent" method="post" action="user.php?frm=pimg_edit" enctype="multipart/form-data" > 
  ...
  <tr valign="top">
	...
	print "<br/>";
	print "&nbsp;<img src='slide/thumb/".$img[$k]."' border='0' align='center'   height='80'  width='118'>"; 
	print "<br/>";
	print "<textarea  name='title2[]' cols='20' rows='3'>".$title[$k++]."</textarea>";
   ...
   print "</table>";
		...
		for($pgno=1;$pgno<$tot_pgs;$pgno++){ 
		  ...
		  print "<a href='user.php?frm=pimg_edit&sel_cat=".$_REQUEST['sel_cat']."&strt=".$strt."&pgn=".$pgno."' class='$currpg'>".$pgno."</a> ";
			$chk_pg=$chk_pg+1;
		}
  ...
  <? if($_SESSION['admin'] != ""){ ?>
    ...
    <input name="btn_update" type="submit" value="Update" class="button2"   > &nbsp;&nbsp;&nbsp;
