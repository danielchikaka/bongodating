...
if(mysql_num_rows(mysql_query("select * from state where country_id='$country' and name='$name' and id!= '$id' "))!= 0)
{	
...
if($errcnt==0)
{
	...
	$sql = " update state set country_id = '$country', name = '$name', abbreviation = '$abbreviation' where id = '$id' ";
	$qry = mysql_query($sql);
		
	$succ = 'State Name edited Successfully!';
...
<?php
$id = $_GET['id'];
$row = mysql_fetch_array(mysql_query("select * from state where id = '$id' "));
    ...
    <td><table width="100%" align="center" border="0" cellspacing="0" cellpadding="2">
        <tr valign="top">
          <td><table width="60%" align="center" border="0" cellspacing="1" cellpadding="2">
              <form name="frmvincent" method="post" action="user.php?frm=state_edit&id=<?=$id?>" enctype="multipart/form-data" onsubmit="return valid();">
                <tr valign="middle">
...
?></td>
			 ...
			 while($confetch=mysql_fetch_array($con)){ ?>
                <option value="<?=$confetch['country_id'];?>" <?php if($country_id==$confetch['country_id']) { echo "selected"; } ?>>
                <?=$confetch['name'];?>
                </option>
                <?php } ?>
              </select></td>
                ...
                <tr>
                  ...
                  <td><input name="btn_save" type="submit" class="button2" value="Submit">
                    <input name="btn_cancel" type="button" onclick="javascript:location.href='user.php?frm=state_list';" class="button2" value="Cancel">
