...
{ 
  ...
  { 
    foreach($_REQUEST['chk_id'] as $id)
	{
	  ...
	  $rs=mysql_query($sql); 
	  $err="Testominal(s) Deleted Successfully !!"; 
	} 
...
{ 
  if(is_array($_REQUEST['chk_id'])){ 
    foreach($_REQUEST['chk_id'] as $id)
	{ 
	  $sql="update team set status='Y' where id='$id'"; 
	  $rs=mysql_query($sql); 
	  $err="Testominal(s) Activated Successfully !!"; 
	} 
  } ?><script>javascript:location.href="user.php?frm=team_edit&err=<?=$err?>";</script><? 
}
...
{ 
  if(is_array($_REQUEST['chk_id'])){
    foreach($_REQUEST['chk_id'] as $id)
	{ 
	  $sql="update team set status='N' where id='$id'"; 
	  $rs=mysql_query($sql); 
	  $err="Testominal(s) DeActivated Successfully !!"; 
	} 
  } ?><script>javascript:location.href="user.php?frm=team_edit&err=<?=$err?>";</script><? 
}
?>
...
<table width="100%" border="0" align="center" cellspacing="0" cellpadding="0">
  <form name="frmvincent" method="post" action="user.php?frm=team_edit" enctype="multipart/form-data"> 
  <tr valign="top">
    <td>
	  <table width="100%" align="center" border="0" cellspacing="1" cellpadding="1">
	    <tr valign="middle">
		  ...
		  <td class="hdrbtn" width="5%" align="center" nowrap="nowrap"><strong>Edit</strong></td>
            ...
            <td width="10%" align="left" class="hdrbtn" style="padding-left:2px;"><strong>Name</strong></td>
           <td class="hdrbtn" align="center" width="47%" style="padding-left:2px;"><strong>Desc</strong></td>
           <td class="hdrbtn" align="center" width="12%" style="padding-left:2px;"><strong>Added date</strong></td>
		  <td class="hdrbtn" align="center" width="12%"><strong>Status</strong></td>
	    </tr>
		<?
		$t_rec=15;
		...
		while($a_row=mysql_fetch_object($rs)){
		  ...
		  if($a_row->status == "Y") $c_status="<strong>Active</strong>"; else $c_status="Inactive"; 
		  ...
		  <img src="../images/edit.gif" onClick="javascript:location.href='user.php?frm=team_modify&id=<?=$id?>';" border="0" style="cursor:pointer;" />
		   <? 
		  print "</td>";
		  
		   print "<td align='center' class='a_txt' style='padding-left:2px;'>".stripslashes($a_row->name)."</td>";
		    print "<td align='center' class='a_txt' style='padding-left:2px;'>".stripslashes($a_row->content)."</td>";
		  print "<td align='center' class='a_txt' style='padding-left:2px;'>".date("m-d-Y",strtotime($a_row->added_date) )."</td>";
		...
		for($pgno=1;$pgno<$tot_pgs;$pgno++){ 
		  ...
		  print "<a href='user.php?frm=team_edit&strt=".$strt."&pgn=".$pgno."' class='$currpg'>".$pgno."</a> ";
			$chk_pg=$chk_pg+1;
		}
