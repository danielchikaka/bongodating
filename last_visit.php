  ...
  <div id="maincont">
    <div id="illust">
      <div class="chemistry_main" style="background:#f1f1f1;">
        <div class="profl_title"><strong>Viewed Me</strong></div>
        <div class="clr"></div>
      </div>
      <?php
// check member who visite your profile and displayed them	  
$visit_check = mysql_query("select * from visitor where user_id='".$_SESSION['userid']."' and status = '1'");
if(mysql_num_rows($visit_check)>0)
{
	while($rowuser = mysql_fetch_array($visit_check)) {
	
	//member details
	$visit_id	= mysql_query("select * from user where user_id='".$rowuser['visitor_id']."'");
	while($row_visitid = mysql_fetch_array($visit_id)){
	
	$user_name 		= $row_visitid['user_name'];
	//$user_image 	= $rowuser['user_image'];
	$user_birthdate = $row_visitid['user_birthdate'];
	$user_gender 	= $row_visitid['user_gender'];
	$user_id 		= $row_visitid['user_id'];
      ...
      <div class="chemistry_main" style="background:#f1f1f1;">
        <div class="chemistry_main" style=" margin-top:20px;">
          ...
          <div class="inbox_txt"><a href="viewprofile.php?profid=<?=$user_id;?>&gen=<?=$user_gender;?>">
            ...
            </a> &nbsp;&nbsp;&nbsp;<?php echo $city;?>, <?php echo $usercountry['name'];?>&nbsp;&nbsp;&nbsp;Last Visit:- <font color="#008000"><?php echo $rowuser['visit_date']; ?></font>
...
?>
      <div class="chemistry_main" style="background:#f1f1f1;">
        <div class="profl_txt_1" style="color:#CC0000;"><strong>Nobody visit on your profile!</strong></div>
      </div>
      <?php } ?>
      <div class="clr"></div>
    </div>
